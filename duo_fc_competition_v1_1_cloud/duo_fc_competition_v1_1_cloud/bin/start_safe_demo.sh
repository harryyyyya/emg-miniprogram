...
if test -n "${DUO_CONFIG:-}"; then
    CONFIG="$DUO_CONFIG"
elif test -r "$ROOT/config/local.env"; then
    CONFIG="$ROOT/config/local.env"
else
    CONFIG="$ROOT/config/mock.env"
fi
...
DUO_MODE="${DUO_MODE:-FULL_CHAIN_SAFE_DEMO}"
case "$DUO_MODE" in
    FULL_CHAIN_SAFE_DEMO|FULL_CHAIN_WITH_EMG_UPLOAD) ;;
    *) echo "FAIL: unsupported DUO_MODE=$DUO_MODE" >&2; exit 2 ;;
esac
export DUO_MODE
...
python3 -m duo_s_full_chain.runtime.main --mode "$DUO_MODE" \
