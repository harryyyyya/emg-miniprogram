...
class NetworkTests(unittest.TestCase):
    def test_http_timeout_can_be_configured_from_environment(self) -> None:
        previous = os.environ.get("DUO_HTTP_TIMEOUT_S")
        os.environ["DUO_HTTP_TIMEOUT_S"] = "5.0"
        try:
            identity = DeploymentIdentity("DUO-S-TEST", "placeholder", "test", "127.0.0.1", 80, "model", "pipeline")
            network = NetworkWorker(ScriptedHttp(), identity, SnapshotStore(RuntimeSnapshot()), StructuredLogger())
            self.assertEqual(network.timeout_s, 5.0)
        finally:
            if previous is None:
                os.environ.pop("DUO_HTTP_TIMEOUT_S", None)
            else:
                os.environ["DUO_HTTP_TIMEOUT_S"] = previous
