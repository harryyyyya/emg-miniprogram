...
class TransportEngineTests(unittest.TestCase):
    def test_runtime_delivers_parsed_frames_to_upload_callback(self) -> None:
        delivered = []
        runtime = FullChainRuntime(
            constant_model(),
            config=RuntimeConfig(mode=RuntimeMode.BLE_INFER_UART_TEST),
            frame_callback=delivered.append,
        )
        runtime.start(0)
        runtime.on_source_connected("ble", 0)
        frame = make_frame(0)
        runtime.notification_callback(frame)
        runtime.poll(0)
        self.assertEqual(len(delivered), 1)
        self.assertEqual(delivered[0].raw, frame)
