...
def main(argv: list[str] | None = None) -> int:
    ...
    def queue_frame_for_upload(frame) -> None:
        nonlocal upload_sequence
        if network is None or not network.collecting:
            return
        snapshot = runtime.snapshots.get()
        network.queue_emg(
            {
                "session_id": network.session_id,
                "gesture_name": f"gesture_{snapshot.gesture_id}",
                "sample_rate_hz": 500,
                "sequence_no": upload_sequence,
                "is_final": False,
                "samples": frame.emg.tolist(),
            }
        )
        upload_sequence += 1
    ...
    runtime = FullChainRuntime(
        ...
        uart_sink=uart,
        logger=logger,
        config=RuntimeConfig(mode=mode),
        frame_callback=queue_frame_for_upload,
    )
