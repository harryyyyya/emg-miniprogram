/* Avoid target libc headers: the installed cross-compiler has no libc-dev. */
...
/*
 * Board build of the F2 feature extractor.  Keep this translation unit free
 * of libc/libm calls so the resulting shared object can be loaded by the
 * Duo S image regardless of the host cross-toolchain's C library.
 */
...
DUO_EXPORT int duo_emg_features(
    ...
    for (channel = 0; channel < 8; ++channel) {
        ...
        }
        {
            ...
            features[8 + channel] = square_root(sum_sq / 125.0f);
